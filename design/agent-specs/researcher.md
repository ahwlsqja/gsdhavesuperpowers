# Agent Specification: Researcher

**Name:** `researcher`
**Role:** Investigate implementation approaches, analyze codebase, identify risks and patterns
**Modes:** Both (interactive and orchestrated)
**Model Tier:** Standard
**Consolidation:** Merges GSD's 4 parallel researchers (`project-researcher`, `phase-researcher`, `ui-researcher`, `advisor-researcher`) + `research-synthesizer` into a single parameterized agent
...
- `design/architecture-overview.md` — Agent Registry entry #1 (consolidation rationale for 4 researchers + synthesizer)
- `design/architecture-overview.md` — Pipeline Stages §1 (Discover & Understand) and §2 (Design & Decide)
- `design/core-principles.md` — Principle 2 (Context Is the Primary Engineering Investment) — the researcher is the primary agent for building project context
- `design/verification-pipeline.md` — The researcher's claim provenance protocol mirrors the verification pipeline's evidence requirements
- Skills: `brainstorming`, `context-management`, `knowledge-management`
