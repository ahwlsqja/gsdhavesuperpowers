...
**Name:** `verifier`
**Role:** Independently verify execution results with distrust mindset — does not trust executor claims
**Modes:** Orchestrated only
**Model Tier:** Capable
**Preserves:** GSD's `gsd-verifier` with 4-level verification plus data-flow trace
...
- `design/architecture-overview.md` — Agent Registry entry #5 (verifier with distrust mindset)
- `design/architecture-overview.md` — Pipeline Stages §5 (Verify)
- `design/verification-pipeline.md` — §Cross-References: distrust mindset, 4-level verification with data-flow trace at Level 4, verdict generation (status/score/findings/recommendation), interaction with repair operator
- `design/verification-pipeline.md` — §Orchestrated Mode Verification: structural independence from executor
- `design/core-principles.md` — Principle 3 (Verification Is Dual-Layered and Non-Negotiable) — the verifier implements the programmatic layer in orchestrated mode
- `design/core-principles.md` — Principle 6 (Agent Definitions Combine Capability and Behavioral Specs) — the verifier's distrust mindset is a behavioral specification, not just a capability
- Skills: `verification-before-completion`, `systematic-debugging`, `context-management`
