# Agent Specification: Auditor

**Name:** `auditor`
**Role:** Specialized auditing: security (STRIDE analysis), UI (6-pillar visual audit), integration checks
**Modes:** Orchestrated only
**Model Tier:** Capable
**Consolidation:** Merges GSD's `gsd-security-auditor`, `gsd-ui-auditor`, `gsd-ui-checker`, `gsd-integration-checker`, and `gsd-nyquist-auditor` into a single parameterized agent. Audit type is specified through prompt parameterization.
...
- `design/architecture-overview.md` — Agent Registry entry #9 (auditor parameterized with security/UI/integration types)
- `design/architecture-overview.md` — Consolidation: GSD's 5 auditor agents → single parameterized agent
- `design/verification-pipeline.md` — The auditor extends the verification pipeline with domain-specific quality checks that the verifier's 4-level process does not cover
- `design/core-principles.md` — Principle 3 (Verification Is Dual-Layered and Non-Negotiable) — the auditor adds domain-specific verification layers on top of functional verification
- `design/core-principles.md` — Principle 7 (Opinionated About Quality) — the auditor enforces domain quality standards (security, visual, integration)
- Skills: `security-enforcement`, `verification-before-completion`, `context-management`
