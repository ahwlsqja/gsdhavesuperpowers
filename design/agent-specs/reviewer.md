...
**Name:** `reviewer`
**Role:** Code quality review with anti-sycophancy protocol — technical pushback required, YAGNI checks
**Modes:** Both (interactive and orchestrated)
**Model Tier:** Capable
**Consolidation:** Merges GSD's `gsd-doc-verifier` (factual claim verification) with Superpowers' `code-reviewer` (6-section review protocol). The anti-sycophancy behavioral specification from the `receiving-code-review` skill is this agent's defining characteristic.
...
- `design/architecture-overview.md` — Agent Registry entry #6 (reviewer with anti-sycophancy protocol)
- `design/architecture-overview.md` — Consolidation: GSD doc-verifier + Superpowers code-reviewer
- `design/verification-pipeline.md` — The reviewer complements the verifier: reviewer assesses quality (is this good code?), verifier assesses correctness (does this work?)
- `design/core-principles.md` — Principle 7 (Opinionated About Quality) — the reviewer is the primary enforcement mechanism for code quality standards
- Skills: `receiving-code-review`, `requesting-code-review`, `verification-before-completion`
