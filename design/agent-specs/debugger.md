...
**Name:** `debugger`
**Role:** Systematic debugging with root-cause-first investigation, structured failure recovery
**Modes:** Interactive only
**Model Tier:** Standard
**Preserves:** GSD's `gsd-debugger` with scientific method debugging, session management, and structured hypothesis testing
...
- `design/architecture-overview.md` — Agent Registry entry #7 (debugger with systematic debugging)
- `design/verification-pipeline.md` — §Failure Recovery: repair operator handles debugging in orchestrated mode; the debugger is the interactive equivalent
- `design/verification-pipeline.md` — §Systematic Debugging Integration: the debugging iron law prevents guess-and-check thrashing during RETRY
- `design/core-principles.md` — Principle 3 (Verification Is Dual-Layered and Non-Negotiable) — debugging is invoked when verification fails
- Skills: `systematic-debugging`, `context-management`, `knowledge-management`
