...
**Name:** `orchestrator`
**Role:** Coordinate multi-agent pipeline: wave analysis, agent spawning, result collection, state management
**Modes:** Orchestrated only
**Model Tier:** Capable
**Design Principle:** Not a spawned agent — this IS the control loop in orchestrated mode. Thin by design: coordinates but does not execute. Implements wave-based parallelism, model resolution, and context enrichment.
...
- `design/architecture-overview.md` — Agent Registry entry #11 (orchestrator as control loop)
- `design/architecture-overview.md` — §Orchestrated Mode: wave-based parallelism, agent spawning, context enrichment, model resolution
- `design/architecture-overview.md` — Infrastructure Services: Wave Coordinator, Agent Spawner, Session Continuity
- `design/verification-pipeline.md` — §Repair Operator: RETRY/DECOMPOSE/PRUNE decision logic, retry budget tracking, cross-task escalation rules, DECOMPOSE task generation protocol
- `design/verification-pipeline.md` — §Escalation Thresholds: maximum verification cycles, context budget guard, cross-task escalation
- `design/core-principles.md` — Principle 1 (One Methodology, Not Two Products) — the orchestrator coordinates the unified methodology, not a GSD-specific or Superpowers-specific pipeline
- `design/core-principles.md` — Principle 5 (Two Modes of Operation) — the orchestrator is the mechanism that enables orchestrated mode
- Skills: `parallel-dispatch`, `context-management`, `using-methodology`
