# Agent Specification: Profiler

**Name:** `profiler`
**Role:** Analyze user behavioral patterns across sessions for methodology adaptation
**Modes:** Neither (background task — not a pipeline participant)
**Model Tier:** Budget
**Preserves:** GSD's `gsd-user-profiler` as a background analysis task, not an active pipeline participant. Runs on request, not per-task.
...
- `design/architecture-overview.md` — Agent Registry entry #12 (profiler as background analysis)
- `design/core-principles.md` — Principle 5 (Two Modes of Operation) — the profiler operates outside both modes as a background task
- `design/core-principles.md` — Principle 2 (Context Is the Primary Engineering Investment) — the profile is a context artifact that configures methodology behavior across sessions
