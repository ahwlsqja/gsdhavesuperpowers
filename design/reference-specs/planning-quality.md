# Reference Specification: Planning Quality

**Name:** `planning-quality`
**Scope:** 8-dimension structural quality checklist for plan verification — the shared quality standard used by the plan-checker agent and the writing-plans skill's self-review checklist
**Loading Behavior:** On demand — loaded by the plan-checker agent during verification and by the planner agent during self-review
**Consumers:** Plan-checker agent (primary — 8-dimension verification), planner agent (self-review before submission), orchestrator agent (plan quality gate decisions)
**Research Grounding:** GSD's `gsd-plan-checker` (8-dimension verification with 3-iteration revision loop); Superpowers' `writing-plans` skill (granularity standard and no-stub mandate); SYN-04 merger
