# Reference Specification: Checkpoint Types

**Name:** `checkpoint-types`
**Scope:** Checkpoint type taxonomy, escalation rules, auto-mode bypass behavior, and checkpoint placement guidelines
**Loading Behavior:** Cold-tier — loaded by planner agent during plan authoring, by executor agent when encountering checkpoint tasks
**Consumers:** Planner agent (checkpoint placement in plans), executor agent (checkpoint protocol execution), orchestrator agent (auto-mode checkpoint handling)
**Research Grounding:** GSD's `checkpoints.md` reference (778 lines); architecture-overview.md §Reference Documents entry 8
